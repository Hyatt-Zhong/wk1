...
int MatchStartCode(unsigned char *pdata) {
    return pdata[0] == 0xFF && (pdata[1] & 0xF0) == 0xF0;
}
 
int GetAACFrame(char* buf, FILE* fp) {
    FILE *bitstream = fp; 
    AdtsFrame frm;
    AdtsFrame *frame = &frm;
    ...
    unsigned char data_buf[AAC_MAX_FRAME_LENGTH];
    ...
    int ret = pdata - pstart;
    memcpy(buf,pstart,ret);
