...
int mov_read_hdlr(struct mov_t* mov, const struct mov_box_t* box)
{
	...
	char descr[30]={0};
	mov_buffer_read(&mov->io, descr,box->size - 24);
	if(0==strcmp(descr,"SoundHandler")) track->handler_descr="SoundHandler";
	if(0==strcmp(descr,"VideoHandler")) track->handler_descr="VideoHandler";
	if(0==strcmp(descr,"SubtitleHandler")) track->handler_descr="SubtitleHandler";
	return 0;
}
