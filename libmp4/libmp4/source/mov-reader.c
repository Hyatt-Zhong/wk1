...
static int mov_read_pw_data(struct mov_t* mov, const struct mov_box_t* box)
{
	assert(box->size==PW_DATA_LEN);
	mov_buffer_read(&mov->io, mov->pw_data, box->size);
	return mov_buffer_error(&mov->io);
}
...
static int mov_index_build(struct mov_track_t* track)
{
	...
	if(!stbl || !track->samples)
	{
		xprint("stbl = 0x%x, samples = 0x%x",stbl, track->samples);
		return 0;
	}
	...
	xprint("stss_count = %lu", stbl->stss_count);
...
//	{ MOV_TAG('s', 't', 'h', 'd'), MOV_MINF, mov_read_default }, // ISO/IEC 14496-12:2015(E) 12.6.2 Subtitle media header (p185)
    ...
    { MOV_TAG('p', 'w', 'd', 't'), MOV_NULL, mov_read_pw_data },
...
int mov_reader_box(struct mov_t* mov, const struct mov_box_t* parent)
{
	...
	while (bytes + 8 < parent->size && 0 == mov_buffer_error(&mov->io))
	{
		...
		else
		{
			...
			// xprint("p1 %llu p2 %llu, size = %llu type = %u",pos,pos2,box.size,box.type);
			// assert(pos2 - pos == box.size);
			if(pos2 - pos != box.size)
			{
				xprint("p1 %llu p2 %llu, size = %llu dt = %llu type = %u",pos,pos2,box.size, pos2-pos, box.type);
				if(box.size > pos2 - pos);
		...
		//读完mdat就结束了
		if (box.type == MOV_TAG('m', 'd', 'a', 't'))
		{
			xprint("mdat end = %llu, mdat size = %llu", mov_buffer_tell(&mov->io), box.size);
			break;
		}
	}
...
void mov_reader_light_destroy(struct mov_reader_t* reader)
{
	// int i;
	// for (i = 0; i < reader->mov.track_count; i++)
	// 	mov_free_track(reader->mov.tracks + i);
...
static struct mov_track_t* mov_reader_next(struct mov_reader_t* reader)
{
	...
	// {
	// 	assert(track2);
	// 	xprint("track2 = 0x%x, i = 0x%x, sample_count = %u", track2, i, track2->sample_count);
	// }
...
FMP4_NEXT_FRAGMENT:
	...
	mov_buffer_read_data(&reader->mov.io, track->handler_type, ptr, sample->bytes);
...
int mov_reader_getinfo(struct mov_reader_t* reader, struct mov_reader_trackinfo_t *ontrack, void* param)
{
	...
	xprint("track_count = %d", reader->mov.track_count);
...
static int mov_stss_seek(struct mov_track_t* track, int64_t *timestamp)
{
	...
	*timestamp = track->samples[idx].dts * 1000 / track->mdhd.timescale;
	...
	xprint("stss_count = %u, sample_offset = %u", track->stbl.stss_count, track->sample_offset);
	return 0;
}
...
static int mov_sample_seek(struct mov_track_t* track, int64_t timestamp)
{
	...
	xprint("sample_count = %u, sample_offset = %u", track->sample_count, track->sample_offset);
	return 0;
}

mov_reader_t* mov_read_copy_reader(mov_reader_t* mp4_rd, struct mov_t* mov_wt)
{
	if (!mp4_rd || !mov_wt)
	{
		return 0;
	...
	mov_reader_t* dest;
	dest = (mov_reader_t*)calloc(1, sizeof(mov_reader_t));
	if (NULL == dest)
		return 0;

	memcpy(&dest->mov,mov_wt,sizeof(struct mov_t));

	dest->mov.io.param = mp4_rd->mov.io.param;
	memcpy(&dest->mov.io.io, &mp4_rd->mov.io.io, sizeof(dest->mov.io.io));

	struct mov_t* mov = &dest->mov;

	mov->track = NULL;
	mov->tracks = calloc(mov->track_count, sizeof(struct mov_track_t));
	memcpy(mov->tracks, mov_wt->tracks, mov->track_count*sizeof(struct mov_track_t));

	for (int i = 0; i < mov->track_count; i++)
	{
		struct mov_track_t* track = mov->tracks + i;
		struct mov_track_t* wt_track = mov_wt->tracks + i;
		struct mov_track_t* rd_track = mp4_rd->mov.tracks + i;

		track->samples = calloc(track->sample_count, sizeof(struct mov_sample_t));
		memcpy(track->samples, wt_track->samples, track->sample_count*sizeof(struct mov_sample_t));
		track->sample_offset = rd_track->sample_offset;

		track->stbl.stss = calloc(1, sizeof(uint32_t));
...
void mov_read_sync_reader(mov_reader_t* dest, struct mov_t* mov_wt)
{
	struct mov_t* mov = &dest->mov;
	...
	for (int i = 0; i < mov->track_count; i++)
	{
		...
		if (!track||!wt_track)
		{
			xprint("track = 0x%x, wt_track = 0x%x", track, wt_track);
			continue;
		}

		track->sample_count = wt_track->sample_count;
		track->samples = realloc(track->samples, track->sample_count * sizeof(struct mov_sample_t));
		assert(track->samples&&track->sample_count);
		memcpy(track->samples, wt_track->samples, track->sample_count*sizeof(struct mov_sample_t));
		//track->sample_offset = track->sample_offset;
...
void mov_read_destroy_copy_reader(mov_reader_t* dest)
{
	if (!dest)
	{
		return;
