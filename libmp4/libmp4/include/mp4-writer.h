...
static inline int mp4_writer_update_free_data(struct mp4_writer_t* mp4, int offset, int len, void* val)
{
    assert(!mp4->fmp4 && mp4->mov);
    if (mp4->mov)
    {
        return mov_writer_update_free_data(mp4->mov,offset,len,val);
    }
    return 0;
}

static inline int mp4_writer_update_moov(struct mp4_writer_t* mp4)
{
    assert(!mp4->fmp4 && mp4->mov);
    if (mp4->mov)
    {
        return mov_writer_update_moov(mp4->mov);
    }
    return 0;
}

static inline struct mp4_writer_t* mp4_writer_create_ex(int is_fmp4, const struct mov_buffer_t* buffer, void* param, int flags, void* mov_rd)
{
    struct mp4_writer_t* mp4;
    mp4 = (struct mp4_writer_t*)calloc(1, sizeof(struct mp4_writer_t));
    if (!mp4) return NULL;
    ...
    assert (!is_fmp4);
    mp4->mov = mov_writer_create_ex(buffer, param, flags, mov_rd); 
